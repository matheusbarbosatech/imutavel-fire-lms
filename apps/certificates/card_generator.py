...
def generate_pvc_card_pdf(user, certificate, base_url="http://127.0.0.1:8000"):
    ...
    pdf_filename = f"carteirinha_{certificate.code}.pdf"
    ...
    # 2. Processar Foto do Aluno
    if getattr(user, 'photo', None) and os.path.exists(user.photo.path):
    ...
    cbmerj = getattr(user, 'cbmerj_registration', 'N/A') or "N/A"
    blood = getattr(user, 'blood_type', 'A+') or "A+"
    ...
    front_data = [
        ...
        [Paragraph(f"CÓDIGO: {certificate.code}", style_hash), qr_reportlab_img]
    ]
    ...
    verso_data = [
        ...
        [Paragraph(
            ...
            "<b>VALIDADE:</b> INDETERMINADA<br/><br/>"
    ...
    # Tabela de Impressão Lado a Lado
